            ...
            <div className="affectation-table-wrapper">
                ...
                <div className="filters-wrapper">
                    <div className="filter-group">
                        ...
                        <button
                            className={statusFilter === 'finished' ? 'filter-btn active' : 'filter-btn'}
                    ...
                    <div className="filter-group">
                        ...
                        <button
                            className={assignmentFilter === 'all' ? 'filter-btn active' : 'filter-btn'}
                            onClick={() => setAssignmentFilter('all')}
                        >
                            Tous
                        </button>
                        <button
                            className={assignmentFilter === 'assigned' ? 'filter-btn active' : 'filter-btn'}
                            onClick={() => setAssignmentFilter('assigned')}
                        >
                            Affectées
                        </button>
                        <button
                            className={assignmentFilter === 'unassigned' ? 'filter-btn active' : 'filter-btn'}
                        ...
                        {filtered.map(cmd => {
                            ...
                            return (
                                <tr key={cmd.id}>
                                    <td>#{cmd.id}</td>
                                    <td>
                                        <span className={`status-badge ${getStatusClass(cmd.status)}`}>
                                            {getStatusLabel(cmd.status)}
                                        </span>
                                    </td>
                                    <td>{new Date(cmd.commandDate).toLocaleDateString('fr-FR')}</td>
                                    <td>
                                        {cmd.status === 'FINISH' ? (
                                                    ...
                                                    <>
                                                        ...
                                                        {missingArticles.map((article, index) => (
                                                            <span key={article.articleId}>
                                                                <Link to={`/stock?articleId=${article.articleId}`}>{article.label}</Link>
                                    ...
                                    <td>
                                        <button
                                            ...
                                            title={cmd.status !== 'WAITING' ? 'Impossible de modifier une commande en cours ou terminée' : ''}
                                        >
                                            {user
