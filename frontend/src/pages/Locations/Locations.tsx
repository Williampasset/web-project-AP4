...
export default function Locations() {
  ...
  const [focusedLocationId, setFocusedLocationId] = useState<number | null>(
    null,
  );
  const [qrModalLocation, setQrModalLocation] = useState<
    (typeof mappedLocations)[number] | null
  >(null);
  const [moveTargetLocationId, setMoveTargetLocationId] = useState<
    number | null
  >(null);
  const [moveQuantity, setMoveQuantity] = useState(1);
  const [mergeTargetArticleId, setMergeTargetArticleId] = useState<
    number | null
  >(null);
  ...
  const mappedLocations = useMemo(() => {
    return [...buildingLocations]
      ...
      .map((location) => {
        const totalStock = location.articles.reduce(
          (sum, article) => sum + article.stock,
          0,
        );
        const hasLowStock = location.articles.some(
  ...
  const renderCell = (cell: (typeof mappedLocations)[number]) => {
    ...
    return (
      <button
        ...
        title={
          label
            ? `${cell.building}${cell.aisle}S${cell.shelf}C${cell.cell} — ${label}`
            : `${cell.building}${cell.aisle}S${cell.shelf}C${cell.cell} — Vide`
        }
      >
        <span className='plan-cell__code'>
          {cell.building}
          {cell.aisle}S{cell.shelf}C{cell.cell}
        </span>
          ...
          {focusedLocation ? (
            <>
              <p className='inspector-ref'>
                {focusedLocation.building}-{focusedLocation.aisle}-
                {focusedLocation.shelf}-{focusedLocation.cell}
                <span
                  className={`zone-badge zone-badge--${focusedLocation.zone.toLowerCase()}`}
                >
              ...
              <p className='inspector-stock'>
                Stock total :{' '}
                {focusedLocation.articles.reduce(
                  (sum, article) => sum + article.stock,
                  0,
                )}{' '}
                unités
                  ...
                  {pendingJobs.map((job) => (
                    <div key={job.id} className='pending-job'>
                      <div className='pending-job__meta'>
                        <strong>#{job.id}</strong> · {job.type} · {job.quantity}
                        u
                        <br />
                        Assigné à {job.assignedUser.firstName}{' '}
                      ...
                      <button
                        ...
                        disabled={isActing}
                        onClick={() =>
                          handleValidateJob(job.id, job.assignedUser.id)
                        }
                      >
              ...
              {focusedArticle && (
                <div className='inspector-actions'>
                  ...
                  {actionError && (
                    <p className='inspector-actions__error'>{actionError}</p>
                  )}
                </div>
              )}
                ...
                {focusedLocation.articles.length === 0 ? (
                  ...
                  focusedLocation.articles.map((article) => (
                    <div key={article.id} className='inspector-article-row'>
                      <div>
                        <span className='inspector-article-ref'>
                          {article.reference}
                        </span>
                        <span className='inspector-article-label'>
                          {article.label}
                        </span>
                      </div>
        ...
        (() => {
          ...
          return (
            <div
              ...
              onClick={() => setQrModalLocation(null)}
            >
              <div className='qr-modal' onClick={(e) => e.stopPropagation()}>
                ...
                <p className='qr-modal__sub'>
                  <span
                    className={`zone-badge zone-badge--${loc.zone.toLowerCase()}`}
                  >
                    {loc.zone}
                  </span>
                  {articleLabel}
